...
class GamepadLogger {
    constructor(logElementId, rosUrl, topicName, messageType) {
    ...
    updateLogEntry(name, value) {
        if (!this.logEntries[name]) {
            const p = document.createElement('p');
            p.id = name;
            p.textContent = `${name}: ${value.toFixed(2)}`;
            this.logDiv.appendChild(p);
            this.logEntries[name] = p;
        } else {
            this.logEntries[name].textContent = `${name}: ${value.toFixed(2)}`;
        }
    }
        ...
        for (let i = 0; i < gamepads.length; i++) {
            ...
            if (gp) {
                ...
                gp.buttons.forEach((button, index) => {
                    ...
                    if (index === 0) { // Button index for toggleButton
                        this.toggleWebButton(pidToggle, pidState);
                    } else if (index === 1) { // Button index for newButton
...
new GamepadLogger('log', 'ws://localhost:9090', '/joystick', 'arcpkg/joystick');
});
