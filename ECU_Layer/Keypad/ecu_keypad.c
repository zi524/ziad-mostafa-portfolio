...
/*for(rows_counter = 0; rows_counter < KEYPAD_ROWS; rows_counter++){
            ...
            ret = gpio_pin_write_logic(&(keypad->rows[rows_counter]), GPIO_HIGH);
            for(columns_counter = 0; columns_counter < KEYPAD_COLUMNS; columns_counter++){
                ret = gpio_pin_read_logic(&(keypad->columns[columns_counter]), &logic);
                if(GPIO_HIGH == logic){
                    *value = keypad_elements[rows_counter][columns_counter];
                }
            }
        }*/